use crate::{soul::Soul, Binding, is_panicking};
use core::{borrow::Borrow, ops::Deref, ptr::NonNull};

/// A `'static` pointer to a value owned by a [`Soul`].
///
/// A `Lich` acts like a `&'static T`, but its validity is dynamically tied to the
/// lifetime of its parent [`Soul`] rather than being statically determined by the
/// Rust compiler. This allows for safely referencing stack-local or other
/// non-`'static` data in contexts that require a `'static` lifetime, such as
/// newly spawned threads.
///
/// # Usage
///
/// A `Lich` is created by calling [`Soul::bind()`] on a pinned [`Soul`]. It can
/// be cloned freely and, depending on the [`Binding`] used, may be sent across
/// threads. It dereferences to the value owned by the `Soul`.
///
/// # Safety
///
/// The core safety mechanism of this type is enforced by the [`Soul`]'s `Drop`
/// implementation. If you attempt to drop a [`Soul`] while one or more of its
/// `Lich`es are still in existence, the `Soul`'s drop will either block the
/// current thread until all `Lich`es are dropped, or it will panic. This
/// behavior depends on the chosen [`Binding`] and guarantees that a `Lich`
/// can never become a dangling pointer to the `Soul`'s data.
